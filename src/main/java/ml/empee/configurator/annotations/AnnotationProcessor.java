...
public final class AnnotationProcessor {
    ...
    static {
        try {
            ...
            throw new RuntimeException("Error while baking AnnotationProcessor reflection", e);
        }
    }

    public static void injectConfigValues(Config config) {
        ...
        if(value != null) {
            ...
            if (parameterClass.isAssignableFrom(value.getClass())) {
                ...
                if(ConfigSection.class.isAssignableFrom(parameterClass) ) {
                    ...
                    value = constructor.newInstance(configField.getConfigPath(), config, configField.isRequired());
                } else {
                    throw MalformedConfigurationException.builder()
                    .message("The config value type doesn't match the parameter class type")
                    .config(config).path(configField.getConfigPath())
                    .build();
                }
            }
    ...
    private static void validateNumericConstraints(BigDecimal value, Constraint constraint, ConfigField configField) {
        if(constraint == null) {
            return;
        }
    ...
    private static void validateStringConstraints(String value, Constraint constraint, ConfigField configField) {
        ...
        if(constraint.notBlank() && value.isBlank()) {
